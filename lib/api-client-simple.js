...
/**
 * Build a credential object from process.env using the spec's env_vars mapping.
 * Returns null if env_vars is not defined or none of the mapped vars are set.
 * @param {Object} authDiscovery - x-postcall-auth-discovery from spec
 * @returns {Object|null}
 */
function loadFromEnvVars(authDiscovery) {
  const envVars = authDiscovery?.env_vars;
  if (!envVars) return null;
  const result = {};
  let anySet = false;
  for (const [fieldName, envVarName] of Object.entries(envVars)) {
    const val = process.env[envVarName]?.trim();
    if (val) { result[fieldName] = val; anySet = true; }
  }
 ...
 */
async function fetchConfluenceCloudId(tokenDataOverride) {
  const tokenData = tokenDataOverride || getToken('confluence');
  if (!tokenData || !tokenData.email || !(tokenData.apiToken || tokenData.api_token)) {
...
async function createClientFromSpec(api, spec, schemaSpec) {
  ...
  if (authDiscovery.type === 'none') {
    ...
    const apiKey = authDiscovery.api_key
      || loadFromEnvVars(authDiscovery)?.api_key
      || getToken(api)?.api_key;
    ...
    const tokenData = loadFromEnvVars(authDiscovery) || getToken(api);
    ...
    if (authDiscovery.cloud_id_discovery) {
      let cloudId = tokenData.cloudId || tokenData.cloud_id;
      if (!cloudId) {
        console.log('      Fetching Cloud ID...');
        try {
          cloudId = await fetchConfluenceCloudId(tokenData);
          if (!envCreds) storeToken(api, { ...tokenData, cloudId }); // only persist token-storage creds
