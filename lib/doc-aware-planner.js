...
async function generateDocAwarePlan(query, matchedOperation, availableOperations = []) {
  ...
  try {
    ...
    const groundingInfo = hasGrounding ? formatGroundingInfo(doc.extensions.grounding, doc.extensions.entityHints) : '';
 ...
 */
function formatGroundingInfo(grounding, entityHints = {}) {
  ...
  if (intentVars.length > 0) {
    ...
    info += `  ${intentVars.map(v => {
      const hint = entityHints && entityHints[v];
      return `${v}: ${hint || `the ${v.replace(/_/g, ' ')} mentioned in the query`}`;
    }).join('\n  ')}\n`;
