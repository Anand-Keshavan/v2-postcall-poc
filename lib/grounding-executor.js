...
async function executeStep(step, spec, client, context) {
  ...
  if (step.filter && data.results) {
    data.results = filterResults(data.results, step.filter);
  }
    ...
    for (const path of Object.values(step.extract)) {
      ...
      if (arrayMatch) {
        ...
        if (Array.isArray(collection) && collection.length === 0) {
          ...
          throw new Error(`No ${collectionName} found matching "${searchedFor}"`);
        }
      }
    }
  }
  ...
  if (step.extract) {
    for (const [key, path] of Object.entries(step.extract)) {
      try {
        ...
        console.error(`[Grounding]   Failed to extract ${key}:`, error.message);
        throw error;
      }
    }
  }
